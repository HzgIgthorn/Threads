...
public class LoopTaskName implements Runnable, Constants {
	...
	public void run() {
		for(int i = 10; i > 0; i-- ) {
			//Das Runnable kann den Namen des Threads ändern, der es aufgerufen hat. Das ist aber
			//in den meisten Fällen keine gute Idee
			Thread.currentThread().setName(name+id);
			thread = Thread.currentThread().getName();
